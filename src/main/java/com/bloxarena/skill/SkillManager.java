...
public class SkillManager {
    ...
    private void updateKitActionBars() {
        ...
        for (Player p : Bukkit.getOnlinePlayers()) {
            ...
            switch (kit) {
                ...
                case ANCHOR -> { if (anchorFields.containsKey(p.getUniqueId())) p.sendActionBar(Component.text("§9⚓ 磁場展開中 §7| 半径8m 減速+弱体+継続ダメ")); }
    ...
    private void anchorSkill(Player p) { setCooldown(p.getUniqueId(),20_000L); anchorFields.put(p.getUniqueId(),p.getLocation().clone()); p.sendMessage("§9§l磁場展開！15秒間 半径8mの敵を減速＋弱体化＋継続ダメ"); p.getWorld().playSound(p.getLocation(),Sound.BLOCK_BEACON_ACTIVATE,1f,0.5f); UUID uid=p.getUniqueId(); new BukkitRunnable(){int count=0;@Override public void run(){if(count++>7||!anchorFields.containsKey(uid)){cancel();return;}Location loc=anchorFields.get(uid);if(loc==null||loc.getWorld()==null){cancel();return;}for(Player t:loc.getWorld().getPlayers()){if(t.getLocation().distance(loc)<=8&&gm.isParticipant(t)&&!gm.isSpectator(t)&&gm.getTeamOf(t)!=gm.getTeam(uid)){t.damage(1.5,Bukkit.getPlayer(uid));}}loc.getWorld().spawnParticle(Particle.PORTAL,loc,30,8,1,8,0.02);}}.runTaskTimer(plugin,40L,40L); Bukkit.getScheduler().runTaskLater(plugin,()->{anchorFields.remove(p.getUniqueId());p.sendMessage("§7磁場終了");},300L); }
