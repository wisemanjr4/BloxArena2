...
public class SkillManager {
    ...
    public void onMegaRocketHit(Location loc, Player shooter) { megaRocketExplode(loc, shooter); }
    ...
    private void berserkerSkill(Player p) { setCooldown(p.getUniqueId(), 18_000L); Vector dir = p.getLocation().getDirection().normalize(); Location cur = p.getLocation().clone(); p.sendMessage("§4§l怒涛爆砕！"); new BukkitRunnable() { int t=0; @Override public void run() { if(t++>10) { cancel(); return; } cur.add(dir.clone().multiply(1.5)); float power = 1.0f - t*0.08f; cur.getWorld().createExplosion(cur, power, false, false, p); cur.getWorld().spawnParticle(Particle.FLAME, cur, 10, 0.5, 0.5, 0.5, 0.03); } }.runTaskTimer(plugin, 2L, 2L); }

    private void pyroSkill(Player p) { setCooldown(p.getUniqueId(), 15_000L); World w = p.getWorld(); Location loc = p.getLocation(); w.playSound(loc, Sound.ENTITY_BLAZE_SHOOT, 1.5f, 0.5f); for (Entity e : w.getNearbyEntities(loc, 5, 3, 5)) { if (!(e instanceof Player t) || !gm.isParticipant(t) || gm.getTeamOf(t)==gm.getTeamOf(p)) continue; if (t.getFireTicks()>0) t.damage(18.0, p); else t.setFireTicks(80); t.getWorld().spawnParticle(Particle.FLAME, t.getLocation().add(0,1,0), 20, 0.3, 0.3, 0.3, 0.05); } }
    ...
    private void whirlwindGust(Player p) { setCooldown(p.getUniqueId(),12_000L); Vector dir=p.getLocation().getDirection().normalize(); Location start=p.getEyeLocation().add(dir.clone().multiply(2)); p.sendMessage("§f§l気流砲！"); new BukkitRunnable(){int ticks=0;Location cur=start.clone();@Override public void run(){if(ticks++>140){cancel();return;}cur.add(dir.clone().multiply(0.5));cur.getWorld().spawnParticle(Particle.CLOUD,cur,8,1.5,1,1.5,0.02);for(Entity e:cur.getWorld().getNearbyEntities(cur,2,2,2)){if(!(e instanceof LivingEntity le)||!le.isValid())continue;if(e instanceof Player tp&&(!gm.isParticipant(tp)||gm.getTeamOf(tp)==gm.getTeamOf(p)))continue;le.setVelocity(dir.clone().multiply(0.3));}}}.runTaskTimer(plugin,0L,2L); }

    public void whirlwindBall(Player p) { if(gm.getPlayerKitType(p.getUniqueId())!=KitType.WHIRLWIND)return; if(isOnCooldown(p.getUniqueId())){Long cd=skillCooldowns.get(p.getUniqueId());long remain=cd!=null?(cd-System.currentTimeMillis())/1000:0;p.sendMessage("§cスキルクールタイム中！ §7残り§f"+remain+"§7秒");return;} setCooldown(p.getUniqueId(),12_000L); Snowball ball=p.launchProjectile(Snowball.class);ball.setVelocity(p.getLocation().getDirection().normalize().multiply(1.0));ball.setCustomName("whirlwindBall");ball.setGlowing(true);p.sendMessage("§f§l旋風弾！");new BukkitRunnable(){int ticks=0;@Override public void run(){if(ticks++>40||!ball.isValid()){cancel();return;}ball.getWorld().spawnParticle(Particle.SWEEP_ATTACK,ball.getLocation(),3,0.3,0.3,0.3,0);Player nearest=null;double nd=16;for(Player t:ball.getWorld().getPlayers()){if(!gm.isParticipant(t)||gm.getTeamOf(t)==gm.getTeamOf(p))continue;double d=t.getLocation().distanceSquared(ball.getLocation());if(d<nd){nd=d;nearest=t;}}if(nearest!=null){Vector toTarget=nearest.getLocation().toVector().subtract(ball.getLocation().toVector()).normalize().multiply(0.4);ball.setVelocity(ball.getVelocity().add(toTarget).normalize().multiply(1.0));}}}.runTaskTimer(plugin,0L,2L); Bukkit.getScheduler().runTaskLater(plugin,()->{for(Entity e:ball.getLocation().getWorld().getNearbyEntities(ball.getLocation(),2,2,2)){if(e instanceof Player t&&gm.isParticipant(t)&&gm.getTeamOf(t)!=gm.getTeamOf(p)){t.setVelocity(new Vector(0,1.5,0));t.damage(3.0,p);}}ball.remove();},40L); }
